...
int cm_prepareClientListMsg(char *msg, int msgLen)
{
	...
	find_wired_client_list(clientListObj);
...
========================================================================
Routine Description:
	Compare two client info is difference or not.

Arguments:
	clientInfo1	- client info 1
	clientInfo2	- client info 2
...
========================================================================
*/
int cm_compareClientInfo(json_object *clientInfo1, json_object *clientInfo2)
{
	int ret = 0;

	if (clientInfo1 && clientInfo2) {
		if (json_object_get_int(clientInfo1) != json_object_get_int(clientInfo2))
			ret = 1;
	}
	else if ((!clientInfo1 && clientInfo2) || (clientInfo1 && !clientInfo2))
		ret = 1;

	return ret;
} /* End of cm_compareClientInfo */
#endif
...
Arguments:
	curClientList	- currenet client list
	newClientList	- new client list
	newClientInfo	- net client info
...
========================================================================
*/
int cm_compareWiredClientlLst(json_object *curClientList, json_object *newClientList, json_object *newClientInfo)
{
	json_object *entryObj = NULL;
	int i = 0, ret = 0, newClientListLen = 0, curClientListLen = 0;
#ifdef RTCONFIG_MULTILAN_CFG
	json_object *curClientInfo = NULL, *infoObj = NULL, *newIdxObj = NULL, *curIdxObj = NULL;
#endif
...
#ifdef RTCONFIG_MULTILAN_CFG
	/* check new client info based on current */
	if ((curClientInfo = json_object_from_file(CURRENT_WIRED_CLIENT_INFO_JSON_PATH))) {
		if (newClientInfo) {
			/* check newClientInfo based on curClientInfo */
			json_object_object_foreach(newClientInfo, newKey, newVal) {
				json_object_object_get_ex(curClientInfo, newKey, &infoObj);
				if (infoObj) {
					json_object_object_get_ex(newVal, CFG_STR_SDN_INDEX, &newIdxObj);
					json_object_object_get_ex(infoObj, CFG_STR_SDN_INDEX, &curIdxObj);
					/* check id */
					if (cm_compareClientInfo(newIdxObj, curIdxObj)) {
						ret = 1;
						break;
					}
				}
			...
			/* check curClientInfo based on newClientInfo */
			if (ret == 0) {
				json_object_object_foreach(curClientInfo, curKey, curVal) {
					json_object_object_get_ex(newClientInfo, curKey, &infoObj);
					if (infoObj) {
						json_object_object_get_ex(curVal, CFG_STR_SDN_INDEX, &curIdxObj);
						json_object_object_get_ex(infoObj, CFG_STR_SDN_INDEX, &newIdxObj);
						/* check id */
						if (cm_compareClientInfo(curIdxObj, newIdxObj)) {
							ret = 1;
							break;
						}
					}
...
int cm_needUpdateWiredClientlLst(json_object *wiredClientList)
{
	...
	int i = 0, ret = 0, newClientListLen = 0, curClientListLen = 0;
	json_object *newClientInfo = NULL;

	newClientListRoot = json_object_new_object();
	if (newClientListRoot) {
		find_wired_client_list(newClientListRoot);
		json_object_object_get_ex(newClientListRoot, CFG_STR_WIRED_MAC, &newClientList);
#ifdef RTCONFIG_MULTILAN_CFG
		json_object_object_get_ex(newClientListRoot, CFG_STR_WIRED_INFO, &newClientInfo);
#endif
		if (wiredClientList) {
			if (newClientList) {
				if (curClientList)
					ret = cm_compareWiredClientlLst(curClientList, newClientList, newClientInfo);
				else
					ret = 1;
...
#ifdef RTCONFIG_MULTILAN_CFG
					if (newClientInfo)
						json_object_to_file(CURRENT_WIRED_CLIENT_INFO_JSON_PATH, newClientInfo);
					else
						unlink(CURRENT_WIRED_CLIENT_INFO_JSON_PATH);
#endif
				}
			}
			else
