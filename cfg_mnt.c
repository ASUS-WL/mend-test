...
#endif /* STA_BIND_AP */
void cm_processWiredClientList(json_object *clientListObj, json_object *clientInfoObj, char *brMac, json_object *onlineListObj, json_object *offlineListObj);
...
void cm_addDutInfo()
{
...
#endif
	json_object *wiredClientList = NULL, *wiredClientInfo = NULL, *capabilityObj = NULL;
	...
	json_object *onlineListObj = NULL, *offlineListObj = NULL;
	...
	if ((wiredClientList = json_object_new_array()) != NULL) {
		if (cm_needUpdateWiredClientlLst(wiredClientList)) {
			if ((onlineListObj = json_object_new_array()) && (offlineListObj = json_object_new_array())) {
#ifdef RTCONFIG_MULTILAN_CFG
				wiredClientInfo = json_object_from_file(CURRENT_WIRED_CLIENT_INFO_JSON_PATH);
#endif
				cm_processWiredClientList(wiredClientList, wiredClientInfo, (char *)get_unique_mac(), onlineListObj, offlineListObj);
#ifdef RTCONFIG_NOTIFICATION_CENTER
				cm_notifyWiredClientEvent(onlineListObj, offlineListObj);
#endif
			}
		...
		json_object_put(wiredClientInfo);
	}
...
int cm_initClientTbl()
{
	...
	cm_addDutInfo();
	cm_updateReInfoToClientTbl(p_client_tbl);
...
void cm_updateBridgeMacList(json_object *wiredMacListObj, char *mac)
{
	...
	if (!neededUpdate) {	/* check all mac between matchedMacListObj and authMacArraryObj */
	...
	if (neededUpdate) {	/* rewrite data to file */
		if (allWiredMacListObj)
...
void cm_record5gBand(int bandNumRE, json_object *channelObj)
{
	...
	if ((selected5gBand != NO_SELECTION) &&
		(bandNumCap == 2 && bandNumRE == 3)) {	/* CAP is dual band, RE is tri-band */
	...
	if (selected5gBand == NO_SELECTION) {
		if (bandNumCap == bandNumRE)	/* supported band is equal */
			return;
		else if (bandNumCap == 2 && bandNumRE == 3) {	/* CAP is dual band, RE is tri-band  */
	...
	if (channelCap != 0) {
		...
		if (strlen(band) > 0) {
			...
			if (!channelObj || !bwObj || !ctrlsbObj
				|| (json_object_get_int(channelObj) != channelCap ||
				json_object_get_int(bwObj) != bwCap ||
				json_object_get_int(ctrlsbObj) != nctrlsbCap)
			) {
			...
			if (sscanf(newBandKey, "%[^_]", newBandIndex) == 1) {
				...
				if (channelObj && bwObj && ctrlsbObj) {
					...
					if (strlen(bandType) > 0) {
						...
						if (!channelObj || !bwObj || !ctrlsbObj
							|| (json_object_get_int(channelObj) != channel ||
							json_object_get_int(bwObj) != bw ||
							json_object_get_int(ctrlsbObj) != nctrlsb)
						) {
							/* delete exited item from newBandObj */
							json_object_object_del(newBandObj, channelKey);
							json_object_object_del(newBandObj, bwKey);
...
#define SZ_NVRAM_UI_REGION_DISABLE		"cfg_ui_region_disable"
	...
	for (i=0; i<p_client_tbl->count; i++)
	{
		disable = (strlen(p_client_tbl->territoryCode[i]) > 0 &&
			...
			cm_isSlaveOnline(p_client_tbl->reportStartTime[i])) ? 1 : 0;
		if (disable == 1) break;
	}
	pthread_mutex_unlock(&cfgLock);
...
========================================================================
Routine Description:
	Update CAP's common & private config.
...
========================================================================
*/
void cm_updateCapCommonPrivateConfig()
{
	json_object *root = NULL, *decryptedRoot = NULL, *commonFtArray = NULL, *privFtArray = NULL, *cfgObj = NULL;
	json_object *ftRoot = NULL, *ftObj = NULL, *ftObjNew = NULL;
	int *commonFtList = cap_common_subft;
	int *privFtList = cap_private_subft;
	char *ftName = NULL, *mac = get_unique_mac();
	unsigned char msgBuf[MAX_PACKET_SIZE] = {0};
	int capFtListLen = 0, commonFtLen = 0, privFtLen = 0;
	...
	/* add common feature */
	if ((commonFtArray = json_object_new_array())) {
		while (*commonFtList != 0) {
			if (!commonFtArray)
				commonFtArray = json_object_new_array();

			if (commonFtArray) {
				if ((ftName = cm_subfeatureIndex2Name(*commonFtList)))
					json_object_array_add(commonFtArray, json_object_new_string(ftName));
			}

			commonFtList++;
		}

		commonFtLen = json_object_array_length(commonFtArray);
		if (commonFtLen > 0) {
			DBG_INFO("commonFtArray(%s)", json_object_to_json_string_ext(commonFtArray, 0));
			json_object_object_add(root, CFG_STR_FEATURE, commonFtArray);
			json_object_to_file(CAP_COMMON_FEATURE_FILE, commonFtArray);
		}
		else
		{
			json_object_put(commonFtArray);
		}
	}

	/* add private feature */
	if ((privFtArray = json_object_new_array())) {
		cm_addDynamicPrivateFeature(privFtArray, supportedBandNum, CAP_SUPPORT);
		while (*privFtList != 0) {
			if ((ftName = cm_subfeatureIndex2Name(*privFtList))) 
				json_object_array_add(privFtArray, json_object_new_string(ftName));
		...
		if (privFtLen > 0) {
			DBG_INFO("privFtArray(%s)", json_object_to_json_string_ext(privFtArray, 0));
			json_object_object_add(root, CFG_STR_PRIVATE_FEATURE, privFtArray);
			json_object_to_file(CAP_PRIVATE_FEATURE_FILE, privFtArray);
		}
		else
		{
	...
	/* record common & private feateure */
	capFtListLen = strlen(json_object_to_json_string_ext(root, 0));
	if (capFtListLen > 0) {
		if ((capFtList = (char *)malloc(capFtListLen + 1))) {
			strlcpy(capFtList, json_object_to_json_string_ext(root, 0), capFtListLen + 1);
			DBG_INFO("cap feature lsit (%s) len(%d)", capFtList, capFtListLen);
		}
		else
		{
			DBG_ERR("malloc failed for capFtList");
		}
	}
	else
	{
		DBG_INFO("capFtListLen is %d", capFtListLen);
	}

	/* update config by private feature */
	if (cm_prepareJoinReturnData(mac, json_object_get_string(root), (char *)&msgBuf[0], sizeof(msgBuf)) > 0) {
		if ((decryptedRoot = json_tokener_parse((char *)msgBuf)) != NULL) {
			...
			if (ftRoot) {
				json_object_object_del(root, CFG_STR_PRIVATE_FEATURE);
				json_object_object_add(root, CFG_STR_PRIVATE_FEATURE, ftRoot);
				cm_handlePrivateCfg(mac, json_object_get_string(root));
			}
		}
	}

	json_object_put(root);
} /* End of cm_updateCapCommonPrivateConfig */
...
========================================================================
*/
static void cm_applyCfgAction(json_object *cfgRoot, int doAction)
{
	struct feature_mapping_s *pFeature = &feature_mapping_list[0];
	json_object *ftObj = NULL, *actionObj = NULL;
	int changed = 0, cfgChanged = 0;
	char actionScript[256] = {0};

	if (cfgRoot) {
		DBG_INFO("config (%s)",  json_object_to_json_string_ext(cfgRoot, 0));
	}
	...
	if (cfgChanged) {
		nvram_commit();
		if (strlen(actionScript)) {
			nvram_set("cap_action", actionScript);
			if (doAction) {
...
int cm_checkFronthaulApUp()
{
	...
	if (nvram_get_int("fh_ap_enabled") == 1) { // Auto
	...
	for (i = 0; i < p_client_tbl->count; i++) {
		if (fh_ap_enabled == 1) { // Auto
			if (p_client_tbl->bandnum[i] >= 3 && cm_isSlaveOnline(p_client_tbl->reportStartTime[i]) && (p_client_tbl->activePath[i] & (WL_2G | WL_5G | WL_5G_1 | WL_6G)) > 0) {
				if ((p_client_tbl->BackhualStatus[i] & 0x1) == 0x0 || (nvram_get_int("fh_ap_bss") == 1 && i == 0)) { // non-use -> use
					if (i == 0) { // CAP
				...
				for (j = 0; j < p_client_tbl->count; j++) {
					if (cm_isSlaveOnline(p_client_tbl->reportStartTime[j])) {
						if ((memcmp(p_client_tbl->pap5g[j], nullMAC, sizeof(nullMAC)) && memcmp(p_client_tbl->ap5g1[i], p_client_tbl->pap5g[j], MAC_LEN) == 0)
							|| (memcmp(p_client_tbl->pap6g[j], nullMAC, sizeof(nullMAC)) && memcmp(p_client_tbl->ap6g[i], p_client_tbl->pap6g[j], MAC_LEN) == 0)) { // 5GH or 6G
						...
						if (memcmp(p_client_tbl->ap5g[i], p_client_tbl->pap5g[j], MAC_LEN) == 0) { // 5GL
						...
						if (memcmp(p_client_tbl->ap2g[i], p_client_tbl->pap2g[j], MAC_LEN) == 0) { // 2.4G
			...
			if ((p_client_tbl->BackhualStatus[i] & 0x1) == 0x1 || (nvram_get_int("fh_ap_bss") == 0 && i == 0)) { // use -> non-use
				if (i == 0) { // CAP
		...
		} else if (fh_ap_enabled == 2) {  //  always on
			if ((p_client_tbl->BackhualStatus[i] & 0x1) == 0x1 || (nvram_get_int("fh_ap_bss") == 0 && i == 0)) { // use -> non-use
				if (i == 0) { // CAP
					p_client_tbl->BackhualStatus[i] = (p_client_tbl->BackhualStatus[i] & 0x0);
					nvram_set_int("fh_ap_bss", 1); // Enable CAP fronthaul AP.
...
static int chk_renotify_backhualstatus()
{
	...
	for (i = 1; i < p_client_tbl->count; i++) {
		...
		if (!cm_isSlaveOnline(p_client_tbl->reportStartTime[i])) // Only check online RE.
			continue;
		if ((p_client_tbl->BackhualStatus[i] >> 3) & 0x1) { // need re-notify.
			ether_etoa(p_client_tbl->realMacAddr[i], macaddr);
...
Arguments:
	...
	*bandNum	- band number for RE
	...
	onlineListObj	- online wired client list
	offlineListObj	- offline wired client list
...
static int cm_updateClientTbl(unsigned char *decryptedMsg, char *clientMac, char *clientIP, int *bandNum, int isFirstJoined, int *newUpdate, int *bh5gSwitch, int *bh5gDiff12Dbm,
	json_object *onlineListObj, json_object *offlineListObj)
{
	json_object *decryptedRoot = json_tokener_parse((char *)decryptedMsg);
	...
	json_object *mocaPrivacyObj = NULL, *passwdObj = NULL, *ePasswdObj = NULL, *privacyEnableObj = NULL, *securityModeObj = NULL;
#ifdef RTCONFIG_AMAS_CENTRAL_CONTROL
...
#ifdef RTCONFIG_MULTILAN_CFG
	json_object_object_get_ex(decryptedRoot, CFG_STR_WIRED_INFO, &wiredInfoObj);
#endif
	...
	if (aliasObj) snprintf(alias, sizeof(alias), "%s", json_object_get_string(aliasObj));
	if (pap2gObj) ether_atoe(json_object_get_string(pap2gObj), pap2g);
	if (pap5gObj) ether_atoe(json_object_get_string(pap5gObj), pap5g);
	if (pap6gObj) ether_atoe(json_object_get_string(pap6gObj), pap6g);
	if (rssi2gObj) rssi2g = atoi(json_object_get_string(rssi2gObj));
	if (rssi5gObj) rssi5g = atoi(json_object_get_string(rssi5gObj));
	if (rssi6gObj) rssi6g = atoi(json_object_get_string(rssi6gObj));
	if (fwVerObj) snprintf(fwVer, sizeof(fwVer), "%s", json_object_get_string(fwVerObj));
	if (swModeObj) swMode = atoi(json_object_get_string(swModeObj));
	if (modelNameObj) snprintf(modelName, sizeof(modelName), "%s", json_object_get_string(modelNameObj));
	if (productIdObj) snprintf(productId, sizeof(productId), "%s", json_object_get_string(productIdObj));
	if (territoryCodeObj) snprintf(territoryCode, sizeof(territoryCode), "%s", json_object_get_string(territoryCodeObj));
	if (bandNumObj) *bandNum = json_object_get_int(bandNumObj);
	if (sta2gObj) {
	...
	if (ap2gSsidObj) strlcpy(ap2gSsid, json_object_get_string(ap2gSsidObj), sizeof(ap2gSsid));
	if (ap5gSsidObj) strlcpy(ap5gSsid, json_object_get_string(ap5gSsidObj), sizeof(ap5gSsid));
	if (ap5g1SsidObj) strlcpy(ap5g1Ssid, json_object_get_string(ap5g1SsidObj), sizeof(ap5g1Ssid));
	if (ap6gSsidObj) strlcpy(ap6gSsid, json_object_get_string(ap6gSsidObj), sizeof(ap6gSsid));
	if (pap2gSsidObj) strlcpy(pap2gSsid, json_object_get_string(pap2gSsidObj), sizeof(pap2gSsid));
	if (pap5gSsidObj) strlcpy(pap5gSsid, json_object_get_string(pap5gSsidObj), sizeof(pap5gSsid));
	if (pap6gSsidObj) strlcpy(pap6gSsid, json_object_get_string(pap6gSsidObj), sizeof(pap6gSsid));
	if (ap2gObj) ether_atoe(json_object_get_string(ap2gObj), ap2g);
	if (ap5gObj) ether_atoe(json_object_get_string(ap5gObj), ap5g);
	if (ap5g1Obj) ether_atoe(json_object_get_string(ap5g1Obj), ap5g1);
#ifdef RTCONFIG_DWB
	if (apdwbObj) ether_atoe(json_object_get_string(apdwbObj), apdwb);
#endif
	if (ap6gObj) ether_atoe(json_object_get_string(ap6gObj), ap6g);
	if (lldpwlcstatObj) strlcpy(lldpwlcstat, json_object_get_string(lldpwlcstatObj), sizeof(lldpwlcstat));
	if (lldpethstatObj) strlcpy(lldpethstat, json_object_get_string(lldpethstatObj), sizeof(lldpethstat));
	if (ap2gFhObj) ether_atoe(json_object_get_string(ap2gFhObj), ap2gFh);
	if (ap5gFhObj) ether_atoe(json_object_get_string(ap5gFhObj), ap5gFh);
	if (ap5g1FhObj) ether_atoe(json_object_get_string(ap5g1FhObj), ap5g1Fh);
	if (ap6gFhObj) ether_atoe(json_object_get_string(ap6gFhObj), ap6gFh);
	if (ap2gSsidFhObj) strlcpy(ap2gSsidFh, json_object_get_string(ap2gSsidFhObj), sizeof(ap2gSsidFh));
	if (ap5gSsidFhObj) strlcpy(ap5gSsidFh, json_object_get_string(ap5gSsidFhObj), sizeof(ap5gSsidFh));
	if (ap5g1SsidFhObj) strlcpy(ap5g1SsidFh, json_object_get_string(ap5g1SsidFhObj), sizeof(ap5g1SsidFh));
	if (ap6gSsidFhObj) strlcpy(ap6gSsidFh, json_object_get_string(ap6gSsidFhObj), sizeof(ap6gSsidFh));
	if (costObj) cost = json_object_get_int(costObj);
	...
	*newUpdate = (found == 1? 0: 1);
	...
	if (found == 0) {	//add client info to table
...
#ifdef RTCONFIG_BHCOST_OPT
		/* update join time */
		if (isFirstJoined) 	p_client_tbl->joinTime[p_client_tbl->count] = uptime();
#endif
		...
		/* update cost */
		if (costObj) p_client_tbl->cost[p_client_tbl->count] = cost;

		p_client_tbl->count++;

		/* update re info */
		cm_updateReInfo(p_client_tbl, realMac);
	...
	else	//update client info to table
	{
...
#ifdef RTCONFIG_BHCOST_OPT
		/* update join time */
		if (isFirstJoined) 	p_client_tbl->joinTime[i] = uptime();
#endif
		...
		else
		{
			if (isFirstJoined) {
				...
				if (rssi5g < 0 && (((rssi5g - p_client_tbl->rssi5g[i]) < OPT_RSSI_DIFF_MINUS)
					|| ((rssi5g - p_client_tbl->rssi5g[i]) > OPT_RSSI_DIFF_PLUS)))
					*bh5gDiff12Dbm = 1;
			}
		}
#endif
		...
		/* update sta */
		if (strlen(sta2gMac) && memcmp(p_client_tbl->sta2g[i], sta2g, MAC_LEN) != 0) {
			memset(p_client_tbl->sta2g[i], 0, MAC_LEN);
			memcpy(p_client_tbl->sta2g[i], sta2g, MAC_LEN);
		}
		if (strlen(sta5gMac) && memcmp(p_client_tbl->sta5g[i], sta5g, MAC_LEN) != 0) {
			memset(p_client_tbl->sta5g[i], 0, MAC_LEN);
			memcpy(p_client_tbl->sta5g[i], sta5g, MAC_LEN);
		}
		if (strlen(sta6gMac) && memcmp(p_client_tbl->sta6g[i], sta6g, MAC_LEN) != 0) {
			memset(p_client_tbl->sta6g[i], 0, MAC_LEN);
			memcpy(p_client_tbl->sta6g[i], sta6g, MAC_LEN);
		}
		...
		/* update band number */
		if (*bandNum > 0)
			p_client_tbl->bandnum[i] = *bandNum;
		...
		/* update cost */
		if (costObj) p_client_tbl->cost[i] = cost;
	}
	...
	if (wiredMacObj && macObj) {
		...
		cm_processWiredClientList(wiredMacObj, wiredInfoObj, realMac, onlineListObj, offlineListObj);
	}

	/* Update bridge's wired mac for DUT */
	dutRootObj = json_object_new_object();
	if (dutRootObj) {
		find_wired_client_list(dutRootObj);
	...
	/* update moca privacy */
	json_object_object_get_ex(decryptedRoot, CFG_STR_MOCA_PRIVACY, &mocaPrivacyObj);
	if (mocaPrivacyObj) {
		json_object_object_get_ex(mocaPrivacyObj, CFG_STR_PRIVACY_ENABLE, &privacyEnableObj);
		json_object_object_get_ex(mocaPrivacyObj, CFG_STR_PASSWD, &passwdObj);
		json_object_object_get_ex(mocaPrivacyObj, CFG_STR_EHANCED_PASSWD, &ePasswdObj);
		json_object_object_get_ex(mocaPrivacyObj, CFG_STR_SECURITY_MODE, &securityModeObj);
		if (privacyEnableObj && passwdObj && ePasswdObj && securityModeObj
			&& !nvram_get("moca_privacy_enable")
			&& !nvram_get("moca_password")
			&& !nvram_get("moca_epassword")
			&& !nvram_get("moca_sceu_mode")
#ifdef RTCONFIG_AMAS_CENTRAL_CONTROL
			&& (paramObj = json_object_new_object())
#endif
		) {
			if (strlen(json_object_get_string(privacyEnableObj))) {
				nvram_set("moca_privacy_enable", json_object_get_string(privacyEnableObj));
#ifdef RTCONFIG_AMAS_CENTRAL_CONTROL
				json_object_object_add(paramObj, "moca_privacy_enable",
					json_object_new_string(nvram_safe_get("moca_privacy_enable")));
#endif
				mocaUpdate = 1;
			}

			if (strlen(json_object_get_string(passwdObj))) {
				nvram_set("moca_password", json_object_get_string(passwdObj));
#ifdef RTCONFIG_AMAS_CENTRAL_CONTROL
				json_object_object_add(paramObj, "moca_password",
					json_object_new_string(nvram_safe_get("moca_password")));
#endif
				mocaUpdate = 1;
			}

			if (strlen(json_object_get_string(ePasswdObj))) {
				nvram_set("moca_epassword", json_object_get_string(ePasswdObj));
#ifdef RTCONFIG_AMAS_CENTRAL_CONTROL
				json_object_object_add(paramObj, "moca_epassword",
					json_object_new_string(nvram_safe_get("moca_epassword")));
#endif
				mocaUpdate = 1;
			}

			if (strlen(json_object_get_string(securityModeObj))) {
				nvram_set("moca_sceu_mode", json_object_get_string(securityModeObj));
#ifdef RTCONFIG_AMAS_CENTRAL_CONTROL
				json_object_object_add(paramObj, "moca_sceu_mode",
					json_object_new_string(nvram_safe_get("moca_sceu_mode")));
#endif
				mocaUpdate = 1;
			}
...
#ifdef RTCONFIG_AMAS_CENTRAL_CONTROL
				if (paramObj)
					cm_updateCommonConfigToFile(paramObj);
#endif
				nvram_commit();
			}
		}
...
#ifdef RTCONFIG_AMAS_CHANNEL_PLAN
	/* update selected channel info */
	json_object_object_get_ex(decryptedRoot, CFG_STR_SELECTED_CHANNEL_INFO, &selChannelInfoObj);
	if (selChannelInfoObj) {
		snprintf(filePath, sizeof(filePath), "%s/%s.sci",
			TEMP_ROOT_PATH, realMac);
		json_object_to_file(filePath, selChannelInfoObj);
	}
#endif
...
static void cm_terminateHandle(int sig)
{
	...
	if (sig == SIGTERM) {
		...
		if (newBandObj) json_object_put(newBandObj);
		if (multiBandObj) json_object_put(multiBandObj);
		...
		if (!IsNULL_PTR(cm_ctrlBlock.publicKey)) MFREE(cm_ctrlBlock.publicKey);
		if (!IsNULL_PTR(cm_ctrlBlock.privateKey)) MFREE(cm_ctrlBlock.privateKey);
...
#if defined(RTCONFIG_BHCOST_OPT) && defined(RTCONFIG_BHSWITCH_RE_SELFOPT)
		if (!IsNULL_PTR(optBhSwitchRule)) MFREE(optBhSwitchRule);
#endif
	}
...
static void cm_usr2Handle(int sig)
{
...
#ifdef RTCONFIG_AMAS_CAP_CONFIG
	json_object *ftObj = NULL, *outFtObj = NULL;
#endif
#endif
	int isCap = 0;
	int applyLock = 0;
...
#ifdef RTCONFIG_AMAS_CAP_CONFIG
	cm_updateCommonConfigToFile(cfgFileObj);
#else
	...
	for (i = 0; i < p_client_tbl->count; i++) {
		...
		if (isCap == 0 && !cm_isSlaveOnline(p_client_tbl->reportStartTime[i])) {
			DBG_INFO("%s (%s) is offline", mac, ip);
			continue;
		}
		...
		if (isCap == 0) {
			...
			nvram_unset("cap_action");
			cm_updateCommonToPrivateConfig(rmac, (unsigned char *)capFtList, cfgFileObj);
			if (capFtList)
				DBG_INFO("cap feature list (%s)", capFtList);
			else
				DBG_INFO("cap feature list is null");

			if ((ftObj = json_tokener_parse(capFtList)) &&
				(outFtObj = json_object_new_object())) {
				/* add params based on feature list */
				cm_addConfigFromCommonFileByFeature(ftObj, outFtObj);
				DBG_INFO("outFtObj (%s)", json_object_to_json_string_ext(outFtObj, 0));

				/* add private feature list */
				cm_addPrivateCfg(rmac, outFtObj);
				DBG_INFO("outFtObj (%s)", json_object_to_json_string_ext(outFtObj, 0));

				/* apply cfg */
				cm_applyCfgAction(outFtObj, 0);
			}

			json_object_put(ftObj);
			json_object_put(outFtObj);
		}
#endif
#endif
	}
...
int ignore_5g2_to_5g1(char *name)
{
	...
	if (ignore == 0) {
		...
		if ((smart_connect == 1 || smart_connect == 3) && strstr(name, "wl1_")) {  // Only for 5G
...
static void cm_transformCfgParam(json_object *inRoot, json_object *outRoot, int private)
{
	...
	struct feature_mapping_s feature_st;
	char word3[64], *next3 = NULL;
	char word4[64], *next4 = NULL;
	char capabilityFilePath[64] = {0};
	char nv_suffix[128];
	char apg_suffix[128];
	char wlsuffix[128];
	...
	json_object *wifiBandNumObj = NULL;
	json_object *featureObj = NULL;
	...
	int band_set = 0;
	int newObj = 0;
	int update_ap_wifi_rl = 0;
	int update_ap_lanif_rl = 0;
	int update_wgn_vlan_rl = 0;
	int apg_idx = 0;
	int exit_loop = 0;

	apg_rule_st apg_rule, apg_rule1;
	apg_rule_st *p_apg_rule = NULL;

	unsigned int wifi_band_num = 0;
	unsigned short apg_band_set = 0;
	unsigned short wifi_band_set = 0;
	char apg_value[2048];
	char vlan_trunk_rulelist[512];
	char vifnames[512];
	char *apgCfg = NULL;
	char apCfgBuf[256];
	char *apCfg = NULL;
	char *re_band_type = NULL;
	char *ptr = NULL;
	char *end = NULL;
	int sdn_vid = -1;
	int sdn_idx = -1;
	int apg_cfg_changed = 0;
	...
	if (uMacObj) {
		...
		if ((capabilityObj = json_object_from_file(capabilityFilePath))) {
			...
			if (json_object_object_get_ex(capabilityObj, s, &wifiBandCapObj) == FALSE)
				wifiBandCapObj = NULL;
		}
	}

	// wifi band num
	json_object_object_get_ex(inRoot, CFG_BAND_TYPE, &wifiBandNumObj);
	wifi_band_num = (wifiBandNumObj==NULL) ? 0 : json_object_get_int64(wifiBandNumObj);
#endif	// RTCONFIG_MULTILAN_CFG
					...
					if (!strcmp(pSubFeature->name, json_object_get_string(ftEntry))) {
						for (pParam = &param_mapping_list[0]; pParam->param != NULL; pParam++) {
							...
							if (pSubFeature->index == pParam->subfeature) {
								...
								if (!cfgAllObj && fileRoot) { /* need to check param in fileRoot */								
#ifdef SUPPORT_TRI_BAND
								...
								if (paramObj) {
...
#if defined(RTCONFIG_MULTILAN_CFG)
									if (strstr(pParam->param, NV_VLAN_TRUNK_RULE)) {
										memset(vlan_trunk_rulelist, 0, sizeof(vlan_trunk_rulelist));
										if (!(apgCfg = create_vlan_trunk_rulelist(uniqueMac, capabilityObj, vlan_trunk_rulelist, sizeof(vlan_trunk_rulelist)-1))) {
											json_object_object_add(paramObj, pParam->param, json_object_new_string(""));
										} else {
											json_object_object_add(paramObj, pParam->param, json_object_new_string((private)?"":apgCfg));
										}
									}
									...
									if (strncmp(pParam->param, "wl", 2) == 0) {
										...
										if (unit > -1 && subunit > 0) {
											found = 0;
											memset(vifname, 0, sizeof(vifname));
											snprintf(vifname, sizeof(vifname)-1, "wl%d.%d", unit, subunit);
											memset(vifnames, 0, sizeof(vifnames));
											if (get_wificap_all_ifnames(uniqueMac, VIF_TYPE_NO_USED, vifnames, sizeof(vifnames)-1, 0)) {
												foreach (word3, vifnames, next3) {
													if (strcmp(vifname, word3) == 0) {
														found = 1;
														break;
													}
												}
											...
											if (!found)
												json_object_object_del(paramObj, pParam->param);
										}
									}
#endif // RTCONFIG_MULTILAN_CFG
#if defined(RTCONFIG_AMAS_WGN)
	...
	if (!cfgAllObj && fileRoot) {
		json_object_object_foreach(fileRoot, k, v) {
			if (!strncmp(k, "apg", 3)) {
				i = -1;
				memset(apg_suffix, 0, sizeof(apg_suffix));
				sscanf(k, "apg%d_%s", &i, apg_suffix);
				if (i <= 0 || strlen(apg_suffix) <= 0)
					continue;
				...
				if ((sdn_vid = get_sdn_vid_by_apg_rule(p_apg_rule)) < 0)
					continue;
				if ((sdn_idx = get_sdn_idx(p_apg_rule->index)) < 0)
					continue;

				if (is_sdn_supported(uniqueMac))
					update_ap_wifi_rl = update_ap_lanif_rl = 1;

				memset(vifnames, 0, sizeof(vifnames));
				if (get_ifnames_used_by_sdn_vid(uniqueMac, sdn_vid, sdn_idx, vifnames, sizeof(vifnames))) {
					foreach (word4, vifnames, next4) {
						memset(nv, 0, sizeof(nv));
						snprintf(nv, sizeof(nv), "%s_bss_enabled", word4);
						newObj = found = 0;
						for (pParam = &param_mapping_list[0]; pParam->param != NULL; pParam++) {
							if (strcmp(pParam->param, nv) == 0) {
								for (pFeature = &feature_mapping_list[0]; pFeature->index != 0; pFeature++) {
						...
						if (found == 1) {
							json_object_object_get_ex(outRoot, pFeature->name, &featureObj);
							if (featureObj)
								newObj = 0;
							else {
							...
							if (featureObj) {
								json_object_object_add(featureObj, pParam->param, json_object_new_string((private)?"":"0"));
								if (pFeature->service)
									json_object_object_add(featureObj, "action_script", json_object_new_string(pFeature->service));
								if (newObj)
									json_object_object_add(outRoot, pFeature->name, featureObj);
								update_apg_ifnames_used(0, uniqueMac, sdn_vid, sdn_idx, get_wifi_band_by_ifname(uniqueMac, word4), word4);
							}
						}
					}
				}
				...
				if (nvram_get_int(nv) == 0 || !get_mtlan_enable_by_idx(i) || !find_mtvlan(sdn_vid))
					continue;

				if (strcmp(apg_suffix, "dut_list") == 0)
					update_ap_lanif_rl = 1;
				
				for (j=0; j<p_apg_rule->dut_list_size; j++) {
					if (strcmp(p_apg_rule->dut_list[j].mac, uniqueMac) != 0)
						continue;

					for (exit_loop=0, x=0; x<WIFI_BAND_ARRAY_SIZE && exit_loop==0; x++) {
						if ((p_apg_rule->dut_list[j].wifi_band & WIFI_BAND_ARRAY[x]) != WIFI_BAND_ARRAY[x])
							continue;

                      	if (!is_sdn_supported(uniqueMac)) {
							memset(apg_value, 0, sizeof(apg_value));
                           	if (num_of_vif_used(uniqueMac, WIFI_BAND_ARRAY[x]) > 0)
	                           	continue;
							if (!get_apg_value(p_apg_rule, WIFI_BAND_ARRAY[x], "auth_mode_x", apg_value, sizeof(apg_value)-1) || strlen(apg_value) <= 0)
								continue;
							if (!is_wgn_auth_mode_supported(apg_value))
								continue;
						}
						...
						memset(vifname, 0, sizeof(vifname));
						if (!is_sdn_supported(uniqueMac))
							snprintf(vifname, sizeof(vifname), "wl%d.1", get_unit_by_band(WIFI_BAND_ARRAY[x]));
						else {
							if (!get_ifname_used_by_band(uniqueMac, sdn_vid,  sdn_idx, WIFI_BAND_ARRAY[x], vifname, sizeof(vifname))) {
								memset(vifname, 0, sizeof(vifname));
								if (!get_unused_ifname_by_dut(uniqueMac, WIFI_BAND_ARRAY[x], vifname, sizeof(vifname)))
									memset(vifname, 0, sizeof(vifname));
							}
						}
						...
						foreach_44 (word3, NV_APG_X_SUFFIX, next3) {
							if (!is_sdn_supported(uniqueMac) && (!strcmp(word3, "radius_port") || !strcmp(word3, "radius_acct_port") || !strcmp(word3, "radius2_port") || !strcmp(word3, "radius2_acct_port")))
								continue;

							newObj = 0;
							memset(nv, 0, sizeof(nv));
							snprintf(nv, sizeof(nv)-1, "%s_%s", vifname, word3);
							found = 0;
							for (pParam = &param_mapping_list[0]; pParam->param != NULL; pParam++) {
								if (strcmp(pParam->param, nv) == 0) {
									for (pFeature = &feature_mapping_list[0]; pFeature->index != 0; pFeature++) {
							...
							if (found == 1) {
								...
								if (featureObj) {
									memset(apg_value, 0, sizeof(apg_value));
									if (get_apg_value(p_apg_rule, WIFI_BAND_ARRAY[x], word3, apg_value, sizeof(apg_value)-1) && strlen(apg_value) > 0) {
										json_object_object_add(featureObj, pParam->param, json_object_new_string((private)?"":apg_value));
										apg_cfg_changed = 1;
									}

									if (apg_cfg_changed == 0) {
										if (newObj == 1) 
											json_object_put(featureObj);
									}
									else {
										if (!is_sdn_supported(uniqueMac))
											update_wgn_vlan_rl = 1;
										else 
											update_ap_wifi_rl = 1;
						...
						if (apg_cfg_changed == 1) {
							newObj = 0;
							memset(nv, 0, sizeof(nv));
							snprintf(nv, sizeof(nv), "%s_bss_enabled", vifname);
							memset(&feature_st, 0, sizeof(struct feature_mapping_s));
							found = 0;
							for (pParam = &param_mapping_list[0]; pParam->param != NULL; pParam++) {
								if (strcmp(pParam->param, nv) == 0) {
									for (pFeature = &feature_mapping_list[0]; pFeature->index != 0; pFeature++) {
							...
							if (found == 1) {
								...
								if (featureObj) {
									json_object_object_add(featureObj, pParam->param, json_object_new_string((private)?"":"1"));
									if (pFeature->service)
										json_object_object_add(featureObj, "action_script", json_object_new_string(pFeature->service));
									if (newObj)
										json_object_object_add(outRoot, pFeature->name, featureObj);
									update_apg_ifnames_used(1, uniqueMac, sdn_vid, sdn_idx, WIFI_BAND_ARRAY[x], vifname);
									if (!is_sdn_supported(uniqueMac)) {
										update_wgn_vlan_rl = 1;
										exit_loop = 1;
									}
		...
		if (cfgAllObj) {
			...
			if (is_sdn_supported(uniqueMac)) {
				...
				if (!get_wgn_all_ifnames(uniqueMac, vifnames, sizeof(vifnames)-1))
					memset(vifnames, 0, sizeof(vifnames));
			}

			if (strlen(vifnames) > 0) {
				foreach (word4, vifnames, next4) {
					memset(nv, 0, sizeof(nv));
					snprintf(nv, sizeof(nv), "%s_bss_enabled", word4);
					newObj = found = 0;
				...
				update_apg_ifnames_used(0, uniqueMac, -1, -1, 0, NULL);
			}
			...
			for (i=1; i<APG_MAXINUM; i++) {				
				...
				for (j=0; j<p_apg_rule->dut_list_size; j++) {
					...
					for (exit_loop=0, x=0; x<WIFI_BAND_ARRAY_SIZE && exit_loop==0; x++) {
						...
						memset(vifname, 0, sizeof(vifname));
						if (!is_sdn_supported(uniqueMac))
							snprintf(vifname, sizeof(vifname)-1, "wl%d.1", get_unit_by_band(WIFI_BAND_ARRAY[x]));
						else {
							if (!get_ifname_used_by_band(uniqueMac, sdn_vid,  sdn_idx, WIFI_BAND_ARRAY[x], vifname, sizeof(vifname))) {
								memset(vifname, 0, sizeof(vifname));
								if (!get_unused_ifname_by_dut(uniqueMac, WIFI_BAND_ARRAY[x], vifname, sizeof(vifname)))
						...
						if (apg_cfg_changed == 1) {
							newObj = 0;
							memset(nv, 0, sizeof(nv));
							snprintf(nv, sizeof(nv)-1, "%s_bss_enabled", vifname);
							memset(&feature_st, 0, sizeof(struct feature_mapping_s));
							found = 0;
							for (pParam = &param_mapping_list[0]; pParam->param != NULL; pParam++) {
								if (strcmp(pParam->param, nv) == 0) {
	...
	// wgn_vlan_rl
	if (update_wgn_vlan_rl == 1) {
		found = 0;
		for (pParam = &param_mapping_list[0]; pParam->param != NULL; pParam++) {
			if (strcmp(pParam->param, NV_WGN_VLAN_RL) == 0) {
				for (pFeature = &feature_mapping_list[0]; pFeature->index != 0; pFeature++) {
					if (pFeature->index == pParam->feature) {
						found = 1;
						break;
		...
		if (found == 1) {
			...
			if (featureObj) {
				memset(apCfgBuf, 0, sizeof(apCfgBuf));
				if ((apCfg = create_wgn_vlan_rl(uniqueMac, apCfgBuf, sizeof(apCfgBuf)-1)) && strlen(apCfg) > 0)
					json_object_object_add(featureObj, pParam->param, json_object_new_string((private)?"":apCfg));
				else 
					json_object_object_add(featureObj, pParam->param, json_object_new_string(""));
				if (pFeature->service)
					json_object_object_add(featureObj, "action_script", json_object_new_string(pFeature->service));
				if (newObj)
					json_object_object_add(outRoot, pFeature->name, featureObj);
	...
	// ap_wifi_rl
	if (update_ap_wifi_rl == 1) {
		found = 0;
		for (pParam = &param_mapping_list[0]; pParam->param != NULL; pParam++) {
			if (strcmp(pParam->param, NV_AP_WIFI_RL) == 0) {
				for (pFeature = &feature_mapping_list[0]; pFeature->index != 0; pFeature++) {
					if (pFeature->index == pParam->feature) {
						found = 1;
						break;
		...
		if (found == 1) {
			...
			if (featureObj) {
				memset(apCfgBuf, 0, sizeof(apCfgBuf));
				if ((apCfg = create_ap_wifi_rl(uniqueMac, apCfgBuf, sizeof(apCfgBuf)-1)) && strlen(apCfg) > 0)
					json_object_object_add(featureObj, pParam->param, json_object_new_string((private)?"":apCfg));
				else 
					json_object_object_add(featureObj, pParam->param, json_object_new_string(""));
				if (pFeature->service)
	...
	// ap_lanif_rl
	if (update_ap_lanif_rl == 1) {
		found = 0;
		for (pParam = &param_mapping_list[0]; pParam->param != NULL; pParam++) {
			if (strcmp(pParam->param, NV_AP_LANIF_RL) == 0) {
				for (pFeature = &feature_mapping_list[0]; pFeature->index != 0; pFeature++) {
					if (pFeature->index == pParam->feature) {
						found = 1;
						break;
		...
		if (found == 1) {
			...
			if (featureObj) {
				memset(apCfgBuf, 0, sizeof(apCfgBuf));
				if ((apCfg = create_ap_lanif_rl(uniqueMac, capabilityObj, apCfgBuf, sizeof(apCfgBuf)-1)) && strlen(apCfg) > 0)
					json_object_object_add(featureObj, pParam->param, json_object_new_string((private)?"":apCfg));
				else 
					json_object_object_add(featureObj, pParam->param, json_object_new_string(""));
				if (pFeature->service)
	...
	if (amas_eap_bhmode_changed == 1 || smart_connect_x_changed == 1) {  // add smart connect config to json.
		int new_obj;
...
{
	int lock;
	json_object *fileRoot = NULL, *oldClientListObj = NULL, *entryObj = NULL;
	...
	time_t ts;
#ifdef RTCONFIG_MULTILAN_CFG
	json_object *wiredClientInfoObj = NULL, *idxObj = NULL;
	int idx = -1;
#endif
	...
	DBG_INFO("brMac (%s), clientListObj (%s)", brMac, json_object_to_json_string_ext(clientListObj, 0));
...
#ifdef RTCONFIG_MULTILAN_CFG
						if (clientInfoObj) {
							json_object_object_get_ex(clientInfoObj, json_object_get_string(entryObj), &wiredClientInfoObj);
							if (wiredClientInfoObj) {
								json_object_object_get_ex(wiredClientInfoObj, CFG_STR_SDN_INDEX, &idxObj);
								if (idxObj) {
...
{
	json_object *wiredClientList = NULL, *wiredClientInfo = NULL;
	json_object *onlineListObj = NULL, *offlineListObj = NULL;
			...
			if ((cfgTmpObj = json_object_from_file(cfgTmpPath))) {
				...
				if ((cfgArrayObj = json_object_new_array())) {
					...
					if (cfgUpdate) {
						...
						cm_updatePrivateRuleByMac(mac, cfgArrayObj, FOLLOW_PRIVATE, RULE_DEL);
					}
				}
...
static void cm_handlePrivateCfg(char *clientMac, unsigned char *decryptedMsg)
{
	...
	int cfgUpdate = 0;
#ifdef RTCONFIG_AMAS_CAP_CONFIG
	int isCap = is_cap_by_mac(clientMac);
#endif
#endif
	...
	if (cfgRoot) {
		...
		if (strlen(cfgTmpPath)) {
			if (!check_if_file_exist(cfgMntPath)) {
				if (strstr(nvram_safe_get("cfg_reoblist"), clientMac)) {
					...
					if ((cfgObj = json_object_new_array()) && cm_transformCfgToArray(cfgRoot, cfgObj)) {
						...
						cm_updatePrivateRuleByMac(clientMac, cfgObj, FOLLOW_PRIVATE, RULE_ADD);
					}
					json_object_put(cfgObj);
				...
				{
					DBG_INFO("add private config to tmp (%s) & mnt (%s)",
			...
			{
				if ((cfgTmpObj = json_object_from_file(cfgTmpPath)) && (cfgObj = json_object_new_object())
					&& (cfgArrayObj = json_object_new_array()))
				{
					...
					if (cfgUpdate) {
						...
						json_object_to_file(cfgTmpPath, cfgTmpObj);
						json_object_to_file(cfgMntPath, cfgTmpObj);
						cm_updatePrivateRuleByMac(clientMac, cfgArrayObj, FOLLOW_PRIVATE, RULE_ADD);
...
#ifdef RTCONFIG_AMAS_CAP_CONFIG
		if (isCap)
			snprintf(privateCfgPath, sizeof(privateCfgPath), "/tmp/cap.json");
		else
#endif
#endif
...
#ifdef RTCONFIG_AMAS_CENTRAL_CONTROL
						if (cm_checkParamFollowRule(clientMac, key, FOLLOW_PRIVATE))
#endif
...
#if defined(RTCONFIG_AMAS_WGN) || defined(RTCONFIG_MULTILAN_CFG)
#ifdef RTCONFIG_MULTILAN_CFG
	if (capabilityObj && !is_sdn_supported(uniqueMac)) {
#else 	
	if (capabilityObj) {
#endif

#ifdef RTCONFIG_MULTILAN_CFG
		wifi_band = get_wifi_band_for_wgn(uniqueMac);
#endif	// RTCONFIG_MULTILAN_CFG
		if (cfgGuestNetworkNo2g)
		{
			...
			if (re_guestif_num > 0)
			{
				...
				if (get_wgn_ifnames(get_unit_by_band(WIFI_BAND_2G), re_guestif_num, tmp_gifnames, sizeof(tmp_gifnames)-1))
...
#endif	/* RTCONFIG_MULTILAN_CFG */
				{
...
#if defined(RTCONFIG_MULTILAN_CFG)
					if (get_wgn_vlan_rl(tmp_gifnames, guest_vlans, sizeof(guest_vlans)-1))
#else 	/* RTCONFIG_MULTILAN_CFG */								
...
#endif	/* RTCONFIG_MULTILAN_CFG */					
					{
						...
						foreach (word, tmp_gifnames, next) {
							...
							if (unit > -1 && subunit > 0) guest_ifnames_ptr += snprintf(guest_ifnames_ptr, guest_ifnames_end-guest_ifnames_ptr, "wl%d.%d ", unit, subunit);
						}
						...
						foreach(word, tmp_gifnames, next)
						{
							...
							if (unit > -1 && subunit > 0)
							{
								...
								if ((wifi_band & WIFI_BAND_2G) == WIFI_BAND_2G)
									json_object_object_add(outRoot, s1, json_object_new_int(1));
								else
									json_object_object_add(outRoot, s1, json_object_new_int(0));
#else   /* RTCONFIG_MULTILAN_CFG */
						...
						foreach (word, tmp_gifnames, next) {
							...
							memset(s2, 0, sizeof(s2));
							if ((wifi_band & WIFI_BAND_2G) == WIFI_BAND_2G)
								snprintf(s2, sizeof(s2), "%d ", 1);
							else 
								snprintf(s2, sizeof(s2), "%d ", 0);
#else	/* RTCONFIG_MULTILAN_CFG */							
							memset(s1, 0, sizeof(s1));
		...
		if (cfgGuestNetworkNo5g)
		{
			...
			if (re_guestif_num > 0)
			{
				memset(tmp_gifnames, 0, sizeof(tmp_gifnames));
#if defined(RTCONFIG_MULTILAN_CFG)				
				if (get_wgn_ifnames(get_unit_by_band(WIFI_BAND_5G), re_guestif_num, tmp_gifnames, sizeof(tmp_gifnames)-1))
#else 	/* RTCONFIG_MULTILAN_CFG */
		...
		if (cfgGuestNetworkNo5gH)
		{
			...
			if (re_guestif_num > 0)
			{
				memset(tmp_gifnames, 0, sizeof(tmp_gifnames));
#if defined(RTCONFIG_MULTILAN_CFG)
				if (get_wgn_ifnames(get_unit_by_band(WIFI_BAND_5GH), re_guestif_num, tmp_gifnames, sizeof(tmp_gifnames)-1))
#else   /* RTCONFIG_MULTILAN_CFG */
		...
		if (cfgGuestNetworkNo6g)
		{
			...
			if (re_guestif_num > 0)
			{
				memset(tmp_gifnames, 0, sizeof(tmp_gifnames));
#if defined(RTCONFIG_MULTILAN_CFG)
				if (get_wgn_ifnames(get_unit_by_band(WIFI_BAND_6G), re_guestif_num, tmp_gifnames, sizeof(tmp_gifnames)-1))
#else   /* RTCONFIG_MULTILAN_CFG */
...
#if defined(RTCONFIG_MULTILAN_CFG)
	json_object_object_add(outRoot, CFG_STR_SDN_SUPPORT, json_object_new_int(1));
#endif	/* RTCONFIG_MULTILAN_CFG */
...
static void cm_updateSlaveRelatedInfo(unsigned char *decryptedMsg)
{
	...
	if (uMacObj) {
		...
		if (capabilityObj) {
			...
			snprintf(infoFilePath, sizeof(infoFilePath), "%s/%s.cap",
				CFG_MNT_FOLDER, json_object_get_string(uMacObj));
			json_object_to_file(infoFilePath, capabilityObj);
...
static int cm_prepareSessionKey(hash_elem_t* e, unsigned char *decryptedMsg, char *msg, int msgLen)
{
	...
	if (e->sessionKeyStartTime > e->sessionKey1StartTime) {
		...
		if (e->sessionKey1) free(e->sessionKey1);
	...
	else
	{
		...
		if (e->sessionKey) free(e->sessionKey);
	...
	if (randKey1) free(randKey1);
	if (randKey2) free(randKey2);
	if (randKey3) free(randKey3);
	...
	else
	{
			...
			gKeyTime >= groupKeyExpireTime))
		{
			...
			if (cm_ctrlBlock.groupKeyStartTime > cm_ctrlBlock.groupKey1StartTime) {
				...
				if (cm_ctrlBlock.groupKey1) free(cm_ctrlBlock.groupKey1);
			...
			{
				/* free groupKey first */
				if (cm_ctrlBlock.groupKey) free(cm_ctrlBlock.groupKey);
...
void cm_updateAPList()
{
	...
	pthread_mutex_lock(&cfgLock);
	for(i = 0; i < p_client_tbl->count; i++) {
		if (memcmp(p_client_tbl->ap2g[i], nullMAC, sizeof(nullMAC)) == 0
			&& memcmp(p_client_tbl->ap5g[i], nullMAC, sizeof(nullMAC)) == 0
			&& memcmp(p_client_tbl->ap5g1[i], nullMAC, sizeof(nullMAC)) == 0
			&& memcmp(p_client_tbl->ap6g[i], nullMAC, sizeof(nullMAC)) == 0)
			continue;
...
static int cm_sendNotification(hash_elem_t* elem, int notifyType, json_object *inData)
{
	...
	DBG_LOG("Send notification request(%d) to client(%s) ... OK", notifyType, elem->clientIP);
...
err:
	if (sock >= 0)
		close(sock);
	DBG_LOG("Send notification request(%d) to client(%s) ... Failed", notifyType, elem->clientIP);
	DBG_INFO("leave");
	return 0;
	...
	for (i = 1; i < p_client_tbl->count; i++) {
		...
		while(e != NULL) {
				...
				strcmp(ip, e->clientIP) == 0)
			{
				DBG_INFO("client ip(%s), client mac(%s)", e->clientIP, e->key);
				e->fwStatus = FW_NONE;
				if (cm_sendNotification(e, NOTIFY_FWCHECK, infoObj)) {  /* ask client to do firmware check */
					e->fwStatus = FW_START;
					ret++;
				}
			}
		...
		while(e != NULL) {
				...
				strcmp(ip, e->clientIP) == 0)
			{
				...
				if (e->fwStatus == FW_SUCCESS_CHECK) {
					e->fwStatus = FW_NONE;
					if (cm_sendNotification(e, NOTIFY_FWDOWNLOAD, NULL)) { /* ask client to do firmware download */
						e->fwStatus = FW_START;
						ret++;
					}
				}
...
void cm_removeSlave(char *mac)
{
	...
	if (found >= 0) {
		...
		DBG_LOG("remove re (%s) in topology", mac);
	}
	else
	{
		DBG_LOG("remove re (%s) not in topology", mac);
	}
	...
	/* remove re lsit */
	cm_updateTribandReList(mac, 0, NULL, RELIST_DEL, 0);
	cm_updateReList(mac, sta2gMac, sta5gMac, sta6gMac, RELIST_DEL);
...
#if defined(RTCONFIG_MULTILAN_CFG)
	update_apg_ifnames_used(0, mac, -1, -1, 0, NULL);
#endif	
...
void cm_removeSlaveRelatedFiles(char *mac)
{
	...
	/* remove capability file in cfg_mnt for slave */
	snprintf(filePath, sizeof(filePath), CFG_MNT_FOLDER"%s.cap", mac);
	if (f_exists(filePath)) {
		DBG_INFO("capability file (%s) in cfg_mnt is removed", filePath);
		unlink(filePath);
	}
...
{
	int i = 0, j = 0, k = 0, listLen = 0, inListLen = 0, notInListLen = 0, needCommit = 0;
	unsigned char ea[6] = {0};
	char ipStr[18] = {0}, macStr[18] = {0}, realMacStr[18] = {0}, mac[18] = {0};
	int found = 0;
	int reObSuccessCount = nvram_get_int("cfg_obcount");
	json_object *macEntry = NULL, *macInTopology = NULL, *macNotInTopology = NULL;
	hash_elem_t *e = NULL;

	macInTopology = json_object_new_array();
	macNotInTopology = json_object_new_array();

	if (macListObj && macInTopology && macNotInTopology) {
		listLen = json_object_array_length(macListObj);

		if (listLen > 0) {	/* for remove re */
			pthread_mutex_lock(&cfgLock);
			for (i = 0; i < listLen; i++) {
				found = 0;
				if ((macEntry = json_object_array_get_idx(macListObj, i))) {
					...
					if (ether_atoe(mac, ea)) {
						...
						if (found) {
							DBG_INFO("add %s in macInTopology", mac);
							json_object_array_add(macInTopology, json_object_new_string(mac));
						}
						else
						{
							DBG_INFO("add %s in macNotInTopology", mac);
							json_object_array_add(macNotInTopology, json_object_new_string(mac));
						}
					}
				}
			}
			...
			/* remove for RE in topology */
			inListLen = json_object_array_length(macInTopology);
			if (inListLen > 0) {
				for (i = p_client_tbl->maxLevel; i >= -1; i--) {
					for (j = 0; j < inListLen; j++) {
						found = 0;
						if ((macEntry = json_object_array_get_idx(macInTopology, j))) {
							strlcpy(mac, json_object_get_string(macEntry), sizeof(mac));

							for (k = 0; k < p_client_tbl->count; k++) {
								if (p_client_tbl->level[k] != i)
									continue;

								memset(macStr, 0, sizeof(macStr));
								memset(realMacStr, 0, sizeof(realMacStr));
								memset(ipStr, 0, sizeof(ipStr));
								snprintf(macStr, sizeof(macStr), "%02X:%02X:%02X:%02X:%02X:%02X",
									p_client_tbl->macAddr[k][0], p_client_tbl->macAddr[k][1],
									p_client_tbl->macAddr[k][2], p_client_tbl->macAddr[k][3],
									p_client_tbl->macAddr[k][4], p_client_tbl->macAddr[k][5]);

								snprintf(realMacStr, sizeof(realMacStr), "%02X:%02X:%02X:%02X:%02X:%02X",
									p_client_tbl->realMacAddr[k][0], p_client_tbl->realMacAddr[k][1],
									p_client_tbl->realMacAddr[k][2], p_client_tbl->realMacAddr[k][3],
									p_client_tbl->realMacAddr[k][4], p_client_tbl->realMacAddr[k][5]);

								snprintf(ipStr, sizeof(ipStr), "%d.%d.%d.%d", p_client_tbl->ipAddr[k][0],
									p_client_tbl->ipAddr[k][1], p_client_tbl->ipAddr[k][2],
									p_client_tbl->ipAddr[k][3]);

								if (strcmp(mac, realMacStr) == 0) {
									found = 1;
									break;
								}
							}
			...
			/* remove for RE not in topology */
			notInListLen = json_object_array_length(macNotInTopology);
			if (notInListLen > 0) {
				for (i = 0; i < notInListLen; i++) {
					if ((macEntry = json_object_array_get_idx(macNotInTopology, i))) {
						strlcpy(mac, json_object_get_string(macEntry), sizeof(mac));
		...
		json_object_put(macInTopology);
		json_object_put(macNotInTopology);
	}
...
void cm_notifyConfigChanged(char *mac)
{
	...
	if (isCap)
		snprintf(cfgPath, sizeof(cfgPath), "/tmp/cap.json");
	else
#endif
	snprintf(cfgPath, sizeof(cfgPath), "/tmp/%s.json", mac);
	...
	if (found) {
		fileRoot = json_object_from_file(cfgPath);
#ifdef RTCONFIG_AMAS_CENTRAL_CONTROL
		...
		if (fileRoot
			...
			json_object_object_foreach(fileRoot, fileKey, fileVal) {
				...
				json_object_object_foreach(paramObj, paramKey, paramVal) {
					...
					if (cm_checkParamFollowRule(mac, paramKey, FOLLOW_COMMON)) {
							...
							if (paramObj) {
								...
								json_object_object_add(fileVal, param,
									json_object_new_string(
										cm_getStrValueFromCommonObj(commonObj, param, valueBuf, sizeof(valueBuf))));
							}
						}
					}
				}
...
#if defined(RTCONFIG_BANDINDEX_NEW)
				json_object_object_add(fileRoot, CFG_BAND_INDEX_VERSION, json_object_new_string("2"));
#else
				json_object_object_add(fileRoot, CFG_BAND_INDEX_VERSION, json_object_new_string("1"));
#endif
			...
			if (newReObj) {
				...
				if (*rssi <= json_object_get_int(rssiObj)) {
					...
					*rssi = json_object_get_int(rssiObj);
					...
					if (rTimeObj) *rTime = json_object_get_int(rTimeObj);
					if (cTimeoutObj) *cTimeout = json_object_get_int(cTimeoutObj);
					if (tTimeoutObj) *tTimeout = json_object_get_int(tTimeoutObj);
				}
...
void cm_handleOnboarding(char *data)
{
	...
	if (json_object_get_int(obStatusObj) == OB_TYPE_AVAILABLE) {
		...
		if (obPath == FROM_NONE) {      /* new RE and related don't be selected */
		...
		else
		{
			...
			if (obPath == FROM_WIRELESS) {
				...
				if (strcmp(reMac, get_re_hwaddr()) == 0) {	/* ob via CAP */
					obViaCap = 1;
				...
				if (cm_checkOnboardingVifCapability(obVifMac)) {
					...
					if (obViaCap == 0) {	/* onboarding via RE */
...
void cm_notifyReboot(json_object *macListObj)
{
	...
	for (i = p_client_tbl->maxLevel; i >= -1; i--) {
		for (j = 0; j < p_client_tbl->count; j++) {
			...
			if (listLen) {	//for specific cap/re
				for (k = 0; k < listLen; k++) {
...
void cm_processFwStatus(char *clientIP, char *clientMac, char *msg)
{
	...
	if (fwStatusObj) {      /* update firmware status of the client in clientHashTable */
...
void cm_processChangedConfigMsg(char *msg)
{
	...
	if (macObj && configObj) {
		snprintf(mac, sizeof(mac), "%s", json_object_get_string(macObj));
		if (cm_checkReListExist(mac))
			cm_updatePrivateConfig(mac, configObj);
	}
...
int cm_processACK_CHK(int sock, CM_CTRL *pCtrlBK, TLV_Header tlv, securityInfo *keyInfo, unsigned char *packetMsg, char *clientIP, char *clientMac)
{
	...
	if (cm_getReMacByIp(p_client_tbl, clientIP, reMac, sizeof(reMac)) > 0) {
		if (!cm_checkObVifReListUpdate(reMac) && cm_getObVifReByNewReMac(reMac, obReMac, sizeof(obReMac))) {
			if (!strcasecmp(obReMac, get_re_hwaddr())) {	/* for CAP */
...
int cm_processREQ_RPT(int sock, CM_CTRL *pCtrlBK, TLV_Header tlv, securityInfo *keyInfo, unsigned char *packetMsg, char *clientIP, char *clientMac)
{
	...
	else
	{
		...
		if (!(onlineListObj = json_object_new_array()) || !(offlineListObj = json_object_new_array())) {
			DBG_ERR("onlineListObj/offlineListObj is NULL !!!");
			if (onlineListObj)
				json_object_put(onlineListObj);
			if (offlineListObj)
				json_object_put(offlineListObj);
	...
	if (
		...
		if (IsNULL_PTR(encryptedMsg)) {
			DBG_ERR("Failed to MALLOC() !!!");
			if (!IsNULL_PTR(decodeMsg)) MFREE(decodeMsg);
...
========================================================================
*/
static int cm_saveUploadFile(char *clientMac, unsigned char *decodedMsg)
{
	int ret = 0;
	FILE_Header fileHdr;
	unsigned char *pData = NULL;
	FILE *fp;
	char fileName[64], folderPath[64], filePath[128];
	unsigned int fileLen = 0;
	char *dump = NULL;

	pData = (unsigned char *)&decodedMsg[0];
	memset(&fileHdr, 0, sizeof(FILE_Header));
	memcpy((unsigned char *)&fileHdr, (unsigned char *)pData, sizeof(FILE_Header));

	/* get file len and name */
	fileLen = ntohl(fileHdr.len);
	strlcpy(fileName, fileHdr.file_name, sizeof(fileName));

	/* shift pointer to the content of file */
	pData += sizeof(FILE_Header);

	/* check corresponding RE's mac folder exist or not */
	snprintf(folderPath, sizeof(folderPath), CFG_MNT_FOLDER"%s", clientMac);
	if (!d_exists(folderPath)) {
		mkdir(folderPath, 0755);
	}

	snprintf(filePath, sizeof(filePath), CFG_MNT_FOLDER"%s/%s", clientMac, fileName);
	DBG_LOG("fileLen(%ld) fileName(%s) filePath(%s)", fileLen, fileName, filePath);

	if ((fp = fopen(filePath, "wb"))) {
		fwrite(pData, 1, fileLen, fp);
		fclose(fp);
		ret = 1;
	}
	else
	{
		DBG_ERR("can't open file (%s)", filePath);
	}

	return ret;
} /* End of cm_saveUploadFile */

/*
========================================================================
Routine Description:
	Process REQ_FILE_UPLOAD packet.
...
========================================================================
*/
int cm_processREQ_FILE_UPLOAD(int sock, CM_CTRL *pCtrlBK, TLV_Header tlv, securityInfo *keyInfo, unsigned char *packetMsg, char *clientIP, char *clientMac)
{
	hash_elem_t* e = NULL;
	unsigned char *encryptedMsg = NULL;
	size_t encryptedMsgLen = 0;
	...
	DBG_INFO("Got REQ_FILE_UPLOAD ...");

	DBG_INFO("Got REQ_FILE_UPLOAD %s %s ...",clientIP,clientMac);
	...
	if (cm_saveUploadFile(clientMac, decodeMsg) == 1) {
		memset(&packetTlvHdr, 0, sizeof(TLV_Header));
		packetTlvHdr.type = htonl(RSP_FILE_UPLOAD);
		MALLOC(encryptedMsg, unsigned char, sizeof(TLV_Header));
		memcpy(encryptedMsg, (unsigned char *)&packetTlvHdr, sizeof(TLV_Header));
		encryptedMsgLen = sizeof(TLV_Header);
	}
	...
	{
		DBG_ERR("Failed to save upload file !!!");
		MFREE(decodeMsg);
		return 0;
	}
	...
	return 1;
} /* End of cm_processREQ_FILE_UPLOAD */
...
void *cm_sendCfgAction(void *args)
{
...
#ifdef RTCONFIG_AMAS_CENTRAL_CONTROL
#ifdef RTCONFIG_AMAS_CAP_CONFIG
	char tmp[64], actionTmp[128], *next;
#endif
#endif
	...
	else
	{
		...
		if (actionScriptObj)
			strlcpy(actionScript, json_object_get_string(actionScriptObj), sizeof(actionScript));
		else
			DBG_INFO("no action script");

#ifdef RTCONFIG_AMAS_CENTRAL_CONTROL
#ifdef RTCONFIG_AMAS_CAP_CONFIG
		strlcpy(actionTmp, nvram_safe_get("cap_action"), sizeof(actionTmp));
		DBG_INFO("cap_action(%s)", actionTmp);
		foreach_59(tmp, actionTmp, next) {
			if (!strstr(actionScript, tmp)) {
				strlcat(actionScript, tmp, sizeof(actionScript));
				strlcat(actionScript, ";", sizeof(actionScript));
			}
		}
		nvram_unset("cap_action");
		...
		if (strlen(actionScript)) {
			DBG_INFO("action script(%s)", actionScript);
			notify_rc(actionScript);
		}

		json_object_put(cfgRoot);
		if (!nvram_get("cfg_nodel_cfg"))
...
void cm_generateGroupKey(CM_CTRL *pCtrlBK)
{
	...
	if (pCtrlBK->groupKey) free(pCtrlBK->groupKey);
...
static int cm_initKeyInfo(CM_CTRL *pCtrlBK)
{
	...
	return 1;
err:
	if (!IsNULL_PTR(pFile)) fclose(pFile);
	if (!IsNULL_PTR(pCtrlBK->publicKey)) MFREE(pCtrlBK->publicKey);
	if (!IsNULL_PTR(pCtrlBK->privateKey)) MFREE(pCtrlBK->privateKey);
	if (!IsNULL_PTR(pCtrlBK->groupKey)) MFREE(pCtrlBK->groupKey);
...
void cm_task(void *pContext)
{
...
#if defined(RTCONFIG_MULTILAN_CFG)
		check_apg_ifnames_used_dir(CFG_MNT_FOLDER);
#endif
	...
	DBG_INFO("copy capability (%s) to /tmp", CFG_MNT_FOLDER"*.cap");
	system("cp "CFG_MNT_FOLDER"*.cap /tmp");

#ifdef RTCONFIG_AMAS_CAP_CONFIG
	cm_updateCapCommonPrivateConfig();
#endif
#endif
#endif
...
========================================================================
*/
int cm_processREQ_CONNDIAG(int sock, CM_CTRL *pCtrlBK, TLV_Header tlv, securityInfo *keyInfo, unsigned char *packetMsg, char *clientIP, char *clientMac)
{
	hash_elem_t* e = NULL;
	unsigned char *decodeMsg = NULL;
	TLV_Header packetTlvHdr;
	...
	memset(&packetTlvHdr, 0, sizeof(packetTlvHdr));
	packetTlvHdr.type = htonl(RSP_CONNDIAG);
	if (write(sock, (char *)&packetTlvHdr, sizeof(TLV_Header)) != sizeof(TLV_Header)) {
		DBG_ERR("Failed to socket write() !!!");
		MFREE(decodeMsg);
		return 0;
...
void *cm_handleOptimization(void *args)
{
	...
	if (nvram_get_int("cfg_opt_stage") >= OPT_STAGE_INIT
		&& nvram_get_int("cfg_opt_stage") <= OPT_STAGE_ADS_NOTIFY_SWITCH) {
		if (optDbg)
	...
	if (p_client_tbl->count > 2 || nvram_get("bh_optmz")) {
		...
		waitDataTimeout = ((nvram_get_int("cfg_opt_ss_time") ? : OPT_SITE_SURVEY_TIME)
							* (nvram_get_int("cfg_opt_ss_times") ? : OPT_SITE_SURVEY_TIMES))
							+ (nvram_get_int("cfg_opt_ss_base_time") ? : OPT_SITE_SURVEY_BASE_TIME);
...
int cm_handleAntennaDiversitySelection(int optTrigger, unsigned int adsTimeStamp, int optmzed, char *nMac)
{
	...
	if (adsPairListObj) {
		...
		if (optTrigger == OPT_TRIGGER_JOIN || optTrigger == OPT_TRIGGER_5G_BACKHAUL_SWITCH
			|| optTrigger == OPT_TRIGGER_5G_RSSI_DIFF_12DBM)
					...
					if (dpsCombNumParent == 1 && dpsCombNum == 1) {
						...
						for (j = 1; j <= dpsCombNumParent; j++) {
							...
							for (k = 1; k <= dpsCombNum; k++) {
								...
								if (iperfDataObj && measureDataObj && switchDataObj
									&& pSeqObj && seqObj)
								{
									...
									if ((sDataObj = json_object_new_object())
										&& (bandObj = json_object_new_object())
										&& (unitObj = json_object_new_object())) {
									...
									if ((mDataObj = json_object_new_object())
										&& (bandObj = json_object_new_object())
										&& (unitObj = json_object_new_object())) {
