...
#endif /* STA_BIND_AP */
void cm_processWiredClientList(char *msg, char *brMac, json_object *onlineListObj, json_object *offlineListObj);
	...
	if ((wiredClientList = json_object_new_array()) != NULL) {
		if (cm_needUpdateWiredClientlLst(wiredClientList)) {
			if ((onlineListObj = json_object_new_array()) && (offlineListObj = json_object_new_array())) {
				cm_processWiredClientList((char *)json_object_to_json_string(wiredClientList), (char *)get_unique_mac(), onlineListObj, offlineListObj);
	...
	json_object *onlineListObj, json_object *offlineListObj)
{
	...
	json_object *plcStatusObj = NULL, *mocaStatusObj = NULL;
	...
	*newUpdate = (found == 1? 0: 1);
	...
	if (wiredMacObj && macObj) {
		...
		cm_processWiredClientList((char *)json_object_to_json_string(wiredMacObj), realMac, onlineListObj, offlineListObj);
	}
	...
	/* update moca status */
	json_object_object_get_ex(decryptedRoot, CFG_STR_MOCA_STATUS, &mocaStatusObj);
	if (mocaStatusObj) {
		snprintf(filePath, sizeof(filePath), "%s/%s.moca",
			TEMP_ROOT_PATH, realMac);
		json_object_to_file(filePath, mocaStatusObj);
	}
	...
	if (json_object_get_int(obStatusObj) == OB_TYPE_AVAILABLE) {
		...
		else
		{
			...
			if (obPath == FROM_WIRELESS) {
				...
				extern int cm_checkOnboardingVifCapability(char *mac);
...
#ifdef RTCONFIG_BHCOST_OPT
	/* update rssi score */
	extern void update_rssiscore(int rssiscore);
	update_rssiscore(0);

    /* set wireless last byte to lldpd */
    extern void set_wifi_lastbyte(void);
    set_wifi_lastbyte();
...
int cm_processREQ_CONNDIAG(int sock, CM_CTRL *pCtrlBK, TLV_Header tlv, securityInfo *keyInfo, unsigned char *packetMsg, char *clientIP, char *clientMac)
{
	...
	if (!IsNULL_PTR(decodeMsg)) {
		extern int cm_convertConnDiagData(unsigned char *inData, char *peerIp, unsigned char *outData, size_t outDataLen);
		extern int cm_sendEventToConnDiag(unsigned char *data);
