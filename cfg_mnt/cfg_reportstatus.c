...
void cm_reportStatus()
{
	...
	if (master) {
		...
		char moca_status_filename[64] = {0};
		...
		for(i = 0; i < p_client_tbl->count; i++) {
			...
			memset(moca_status_filename, 0, sizeof(moca_status_filename));
			...
			/* moca status */
			snprintf(moca_status_filename, sizeof(moca_status_filename), TEMP_ROOT_PATH"/%s.moca", rmac_buf);
			if (f_exists(moca_status_filename))
				mocaStatusObj = json_object_from_file(moca_status_filename);
			...
			if (clientObj) {
				...
				if (mocaStatusObj)
					json_object_object_add(clientObj, CFG_STR_MOCA_STATUS, mocaStatusObj);
