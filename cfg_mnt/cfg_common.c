...
Arguments:
	...
	*portIndex		- which port index need to be processed
...
========================================================================
*/
void add_brforward_entry_by_port(json_object *root, char *portIndex)
{
	FILE *fd;
	int i, cnt;
	char path[128] = {0};
	struct __fdb_entry fe[BR_MAX_ENTRY];
	json_object *brArry = NULL;
	char mac[18] = {0};
	char port_buf[32] = {0};
	int port_no = 0;
	char word[16], *next;
	...
	snprintf(path, PATH_MAX, "/sys/class/net/%s/brforward", nvram_safe_get("lan_ifname"));
	fd = fopen(path, "r");
	...
	if (fd) {
		...
		snprintf(port_buf, sizeof(port_buf), "%s", portIndex);
			...
			for (i = 0; i < cnt; i++) {
				if (fe[i].port_no == port_no) {
					...
					if (nvram_get("cfg_brd")) {
						DBG_INFO("mac_addr(%s) port_no(%d) is_local(%d), ageing sec(%d)",
					...
					if (fe[i].is_local == 0) {
						...
						if ((is_wired_re && (ageing <= re_ageing_timer)) ||
							...
							if (!brArry)
								brArry = json_object_new_array();

							if (brArry)
								json_object_array_add(brArry, json_object_new_string(mac));
						}
					}
				}
			}
		...
		if (brArry)
			json_object_object_add(root, CFG_STR_WIRED_MAC, brArry);
	}
} /* End of add_brforward_entry_by_port */
...
#if defined(RTCONFIG_BHCOST_OPT)
	if (amas_path_stat == ETH && get_type_by_ifname(amas_ifname) != ETH_TYPE_PLC && get_type_by_ifname(amas_ifname) != ETH_TYPE_MOCA) {
...
#ifdef RTCONFIG_MOCA
			link_rate_int = get_uplinkports_linkrate(amas_ifname, node);
#else
...
#ifdef RTCONFIG_MOCA
int get_moca_status(json_object *moca_status)
{
	int ret = 0;
	...
	char amas_ifname[16], mode[8] = {0}, eth[32]={0}, *next = NULL, nvram_buf[] = "amas_ethXXX_linkrate";
	int amas_path_stat = nvram_get_int("amas_path_stat");
	int i = 0;
	...
	if (moca_status == NULL){
		DBG_ERR("moca_status is NULL");
		return 0;
	}
	if (amas_path_stat == ETH && get_type_by_ifname(amas_ifname) == ETH_TYPE_MOCA) {
		link_rate = get_uplinkports_linkrate(amas_ifname, node);
		foreach(eth, nvram_safe_get("eth_ifnames"), next)
		{
			if (strcmp(eth, amas_ifname) == 0)
				break;
			i++;
		}
		snprintf(nvram_buf, sizeof(nvram_buf), "amas_eth%d_linkrate", i);
		link_rate = nvram_get_int(nvram_buf);
		DBG_INFO("link rate (%d)", link_rate);

		json_object_object_add(moca_status, CFG_STR_LINK_RATE, json_object_new_int(link_rate));
		ret = 1;
	}
#endif
...
int check_match_6G(char *mac)
{
	...
	if(re_have_6G>0){
		...
		foreach (word, wlIfnames, next) {
				...
				//DBG_INFO(" nband=%s ",prefix);
				if (nband == 4) {
					DBG_INFO("cap  have 6G and re does not have 6G");
					json_object_put(fileRoot);
					return 2;	
				}
				i++;
		}
		DBG_INFO("!Both cap and re does not have 6G");
		json_object_put(fileRoot);
		return 0;
	}
}
